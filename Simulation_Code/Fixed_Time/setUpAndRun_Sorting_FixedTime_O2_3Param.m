...
% each parameter to its specific variable later on inside the for loop)
    ...
    % Set the parameter values which will be updated in the for loop
        % % Low resolution around all possible parameter choices
        %     % Homotypic adhesion energy in multiples of laboratory kT
        %         parameterVals1 = 10.^[3, 4, 5, 6, 7, 8];
        %     % Viscosity of the tissue (in Pa s; water = 10^(-3) Pa s, Early zfish embryo max = 10^3)
        %         parameterVals2 = 10.^(-3:3);
        %     % Effective temperature in multiples of E_homo
        %         parameterVals3 = linspace(0,2,41);
        % High resolution around experimental conditions
            % Homotypic adhesion energy in multiples of laboratory kT 
                parameterVals1 = 10.^(3:0.2:7);
            % Viscosity of the tissue (in Pa s; water = 10^(-3) Pa s, Early zfish embryo max = 10^3)
                parameterVals2 = 1;
            % Effective temperature in multiples of E_homo
                parameterVals3 = 0.25:0.05:1.5;
