function [outputVars] = simSorting_FeedFile_Ising_GlobalSwap(inputFilePath,jobID,taskID,timeLimit)
...
%% Set up the simulation
    ...
    % Initialize the simulation
        ...
        % Calculate the number of time steps
            numTimeSteps = 1000*numel(cellType_onGrid);
            tp2Record = 2;
...
for stepNum = 1:numTimeSteps
    ...
    % Pick another random cell
        NN_linIdx = randi(numel(cellType_onGrid));
    % Find its row and column indices 
        NN_row = target_rows(NN_linIdx);
        NN_col = target_cols(NN_linIdx);
    ...
    % Calculate the change in energy due to the switch
        if cellType_onGrid(target_row, target_col)==cellType_onGrid(NN_row, NN_col)
            ...
            % Calculate the change in energy if they were to switch
                ...
                if any(sum([target_above,target_col; target_row,target_left; target_row,target_right; target_below,target_col]==[NN_row, NN_col],2)==2)
                    % If the cells are neighbors, then only the 6 of their
                    % neighbors will be swapped (their shared interface will remain the same)
                    dE = (6-(2*numSameCellType_Current))*(E_homo-E_het);
                else
                    % If the cells are neighbors, then all 8 neighbors will
                    % be swapped
                    dE = (8-(2*numSameCellType_Current))*(E_homo-E_het);
                end
        end
