...
% simSorting_FeedFile_FixedRecTime.m,simSorting_FeedFile_Ising_GlobalSwap.m, or
% simSorting_FeedFile_Ising_LocalSwap.m. You have the option to iterate 
...
% each parameter to its specific variable later on inside the for loop)
    ...
    % Set the parameter values which will be updated in the for loop
        % Comparson between discrete times steps, local MC, or global MC
            % Homotypic adhesion energy in multiples of laboratory kT
                parameterVals1 = 10^8;
            % Viscosity of the tissue relative to water (water = 10^(-3) Pa s, Early zfish embryo max = 10^3)
                parameterVals2 = 10;
            % Effective temperature in multiples of E_homo
                parameterVals3 = [0.25 0.5 0.75 1 1.25 1.5 1.75 2];
            % Box size
                parameterVals4 = [150];
...
% Choose where to find/save variables
    ...
    % needed)
        fileNameForSprintf = 'Results_%s_SortSim_MCTest_%i_Rep_%i_in';            
...
for parameterValsNum = 1:length(globalInfo.parameterVals)
    ...
    if parameterValsNum==1
        % Copy the functions necessary to run this script into the folder
            ...
            copyfile(which('simSorting_FeedFile_Ising_New.m'),globalInfo.folderPathOnLocal)     
    end
    ...
    % that need to be chosen randomly, like the initial filament positions)
        ...
        % Adjust whichever parameters we're scanning through
            % Make the box smaller
                sizeX = globalInfo.parameterVals4(globalInfo.paramCombos(parameterValsNum,4));
                sizeY = sizeX;
