...
% Choose which parameters to plot
    ...
    % Choose the representative values of kT to emphasize
        kTVals2Emphasize = [0.25 0.5 0.75 1 1.25]; 
    ...
    % Sorting over time
     ...
     % Save the figure 
        % Choose the filename and path for the figure
            destinationTrack = [figOutputFolderPath 'Fig2ei_Fig2SuppFig1a_SortingVisualVskT'];
...
%% For these files, plot the endpoint
    ...
    for kTNum2Emphasize = 1:length(kTVals2Emphasize)
        ...
        % Plot the endpoint
            figure(6)
            subplot(2,length(kTVals2Emphasize),matFileCounter)
            imagesc(squeeze(cellID_onGrid_OverTime(:,:,end)))
            title(sprintf('k_BT_M = %.2f k_BT_L',kT_All(matfileNum2Plot)))
            set(gca,'FontName','Helvetica','FontSize',5)
            axis equal
            xlim([0.5 50.5])
            ylim([0.5 50.5])
            subplot(2,length(kTVals2Emphasize),length(kTVals2Emphasize) + matFileCounter)
     ...
     % Save the figure 
        % Choose the filename and path for the figure
            destinationTrack = [figOutputFolderPath 'Fig2m_SortingVisualVsKT'];
        % Choose the figure size and position
            figHandle = figure(6);
